...
const run = async options => {
	...
	let importFromProject = process.env.AVA_IMPORT_FROM_PROJECT_NO_STUB === '1'
		? async ref => {
			// Resolve the module from the project's perspective without writing a stub
			// file to node_modules/.cache/ava. Useful in CI environments where
			// node_modules is read-only. Note: require.resolve() uses the "require"
			// export condition, which may differ from the "import" condition for
			// packages with conditional exports.
			const projectRequire = createRequire(path.join(projectDir, 'package.json'));
			return import(pathToFileURL(projectRequire.resolve(ref)));
		}
		: async ref => {
